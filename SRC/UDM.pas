...
const
  ...
  sDBALIAS='ALIAS_SHHJ';
  CGYXJB='常规';//常规优先级别字符串
  SHOW_CHK_CON_HIS='select cch.unid as 唯一编号,cch.lsh as 样本号,cch.patientname as 姓名,cch.report_doctor as 审核者,'+
        ...
        ' cch.issure as 备注,cch.combin_id as 申请单工作组,cch.germname as 细菌,cch.checkid as 联机号, '+
...
begin
  ...
  begin
    ss:='服务器'+#2+'Edit'+#2+#2+'0'+#2+#2+#3+
        ...
        '口令'+#2+'Edit'+#2+#2+'0'+#2+#2+'1'+CryptStr;
