...
uses  UDM;
const
  sqlstr=' SELECT  id AS 用户代码, name AS 用户名称, pinyin AS 英文名,Telphone as 电话号码, unid AS 唯一编号'+
...
Begin
   ...
   begin
        ...
        adotemp12.SQL.Text:='Insert into WORKER ('+
                            ' Name,PassWd,account_limit,pkdeptid,ID,Telphone) values ('+
                            ':P_Name,:P_PassWd,:P_account_limit,:P_pkdeptid,:P_ID,:P_Telphone) ';
        ...
        adotemp12.Parameters.ParamByName('P_Telphone').Value:=trim(uppercase(LabeledEdit3.Text)) ;
        adotemp12.ExecSQL;
        adotemp12.Free;
   ...
   begin
        ...
        adotemp11.Close;
        adotemp11.SQL.Clear;
        adotemp11.SQL.Text:='update worker set id=:id,name=:name,Telphone=:Telphone '+
                            'where unid=:unid';
        try
        ...
        adotemp11.Parameters.ParamByName('Telphone').Value:=trim(uppercase(LabeledEdit3.Text));
...
begin
        ...
        begin
            ...
            LabeledEdit3.Text:=trim(ADOdoclist.FieldByName('电话号码').AsString);
        end else
        begin
            LabeledEdit1.Clear;
            LabeledEdit2.Clear;
